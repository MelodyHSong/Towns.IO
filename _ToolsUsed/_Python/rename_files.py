...
# ☆ Date: 2025-12-13
# ☆
# ☆ Description: Renames image files based on municipality and region, 
...
# We only care about file types that are shields (which should be PNG or SVG, based on the previous script)
IMAGE_EXTENSIONS = {'.png', '.svg' , '.jpg', '.jpeg', '.gif', '.bmp', '.tiff', '.webp'}

# ☆ Description: Data structure derived from the Kotlin TownDataSource.kt file.
TOWNS_DATA = [
...
# ☆ Description: Creates a dictionary mapping normalized municipality name to the desired new snake_case prefix.
    ...
    for town in TOWNS_DATA:
        ...
        # 3. Create the final prefix: {region}_{name}_shield
        region_lower = region.lower()
        new_prefix = f"{region_lower}_{name_snake_case}_shield"
        
        # The key is the standardized, unaccented, lowercased version of the municipality name
        # This will be used to match against the files
...
# ☆ Description: Prompts the user to select between a Dry Run (safe) or a Normal Run (delete) with a confirmation for deletion.
    print("\n--- RUN MODE SELECTION ---")
    print("Do you want to run the script in DRY RUN mode (no files deleted/renamed)?")
...
# ☆ Description: Main function to process and rename the files.
