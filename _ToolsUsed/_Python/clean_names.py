...
# ☆ Author: ☆ MelodyHSong ☆
# ☆ Language: Python
# ☆ File Name: municipality_shield_cleanup.py
# ☆ Date: 2025-12-13
# ☆
# ☆ Description: Cleans up duplicate image files based on Puerto Rican municipality names,
# ☆ prioritizes PNGs, then SVGs, and keeps the largest file size. Also lists missing shields.
# ☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆
...
# ☆ Description: Helper function to remove accents and punctuation for robust matching.
    ...
    # Remove all combining characters (the accents/diacritics)
    text = ''.join(char for char in text if unicodedata.category(char) != 'Mn')
    # Convert to lowercase and remove common delimiters like hyphens, periods, etc.
...
# ☆ Description: Checks if any municipality name is in the filename using robust normalization.
    ...
    for muni in MUNICIPALITIES:
        ...
        # Check if the normalized municipality name is a substring of the normalized file name
        if normalized_muni in normalized_file_name:
            # Found a match, return the original, correct municipality name
            return muni 
...
# ☆ Description: Prompts the user to select between a Dry Run (safe) or a Normal Run (delete) with a confirmation for deletion.
    ...
    while True:
        ...
        if choice == 'Y':
            ...
            # Add the crucial warning and final confirmation
            print("\n!!! DANGER ZONE !!!")
...
# ☆ Description: Main function to clean duplicate files and report missing shields.
    ...
    found_municipalities = set() # New set to track found names
    files = [f for f in TARGET_FOLDER.iterdir() if f.is_file() and f.suffix.lower() in IMAGE_EXTENSIONS]

    print(f"\nScanning {len(files)} images in '{TARGET_FOLDER.name}'...\n")
    
    for f in files:
        muni_name = find_municipality_in_name(f.name)
        if muni_name:
            municipality_groups[muni_name].append(f)
            found_municipalities.add(muni_name) # Add the found name
        # else: File without a municipality name is ignored for this cleanup
    ...
    for muni, group in municipality_groups.items():
        ...
        print(f"--- PR Municipality: {muni} (Found {len(group)} files) ---") 
        
        # Sort the group based on the priority logic
    ...
    if missing_municipalities:
        print("☆☆ MUNICIPALITY SHIELDS MISSING ☆☆")
        print(f"You are missing shields for {len(missing_municipalities)} municipalities:")
        for muni in missing_municipalities:
            print(f" * {muni}")
        print("-" * 50)
    else:
        print("☆☆ CONGRATULATIONS! ALL 78 MUNICIPALITY SHIELDS WERE FOUND! ☆☆")
...
if __name__ == "__main__":
    # Get run mode from user prompt
    should_dry_run = get_run_mode_from_user()
    
    # Execute the cleanup function with the user's choice
